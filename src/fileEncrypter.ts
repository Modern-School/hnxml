...
export function getHashCode(str: string): number {
  ...
  for (; len > 2; len -= 4, i += 4) {
    ...
    const c1 = (i + 1 < str.length ? str.charCodeAt(i + 1) : 0) | 0;
    const c2 = (i + 2 < str.length ? str.charCodeAt(i + 2) : 0) | 0;
    const c3 = (i + 3 < str.length ? str.charCodeAt(i + 3) : 0) | 0;

    const val1 = (c0 | (c1 << 16)) | 0; // *ptr2
    const val2 = (c2 | (c3 << 16)) | 0; // ptr2[1]

    // 关键：右移必须用 >>（算术右移），与 C# int 一致
    num = (((num << 5) + num + (num >> 27)) ^ val1) | 0;
    num2 = (((num2 << 5) + num2 + (num2 >> 27)) ^ val2) | 0;
  }
  ...
  if (len > 0) {
    const c0 = str.charCodeAt(i) | 0;
    const c1 = (i + 1 < str.length ? str.charCodeAt(i + 1) : 0) | 0;
    const val = (c0 | (c1 << 16)) | 0;
    num = (((num << 5) + num + (num >> 27)) ^ val) | 0;
  }

  // 关键：32位乘法用 Math.imul，得到与 C# 相同的溢出结果
  return (num + Math.imul(num2, 1566083941)) | 0;
  // 如果你需要“无符号 int”形式的结果（UInt32），可以这样返回：
  // return (num + Math.imul(num2, 1566083941)) >>> 0;
}
...
export const hashCollisionCheck = (pass: string): boolean =>
  Math.trunc(int2Ushort(getHashCode(pass)) / 1822) === 3;
